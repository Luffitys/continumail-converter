...
public class MboxParserBoundaryTests
{
    ...
    [Fact]
    public void Parse_JunkDayMonthPostmarkAfterNonBlankLine_IsNotSplit()
    {
        // "From x foo bar 12 10:00:00 2020" matches the OLD permissive regex (foo/bar as bare
        // \w{3}) but foo/bar are not real day/month names. On the no-blank-separator fallback
        // path it would wrongly split; with day/month validation it stays body content.
        string content =
            "From real@example.com Mon Jan  1 00:00:00 2024\n" +
            "Subject: Real\n" +
            "\n" +
            "Body line one.\n" +
            "From x foo bar 12 10:00:00 2020\n" +   // NOT preceded by a blank line
            "Body line three.\n";
        string path = WriteTempMbox(content);
        try
        {
            var parser = new MboxParser();
            var results = parser.Parse(path).ToList();
            Assert.Single(results);                                          // one message, not two
            Assert.Equal("Real", results[0].Message!.Subject);              // captured as one whole message
            Assert.Contains("From x foo bar 12 10:00:00 2020", results[0].Message!.TextBody);
            Assert.Equal(1, parser.CountMessages(path));
        }
        finally { File.Delete(path); }
    }
    ...
    [Fact]
    public void Parse_JunkDayMonthPostmarkAfterBlankLine_StillSplits()
    {
        // Scoping guard: the regex governs ONLY the no-blank path. The SAME junk-shaped line,
        // preceded by a blank line, is still a boundary (mboxrd blank-line + "From ")
        // regardless of the regex. Tightening the regex must not change this.
        string content =
            "From a@example.com Mon Jan  1 00:00:00 2024\n" +
            "Subject: One\n" +
            "\n" +
            "Body one.\n" +
            "\n" +                                  // blank line BEFORE the junk line
            "From x foo bar 12 10:00:00 2020\n" +
            "Subject: Two\n" +
            "\n" +
            "Body two.\n";
        ...
        try
        {
            ...
            Assert.Equal(2, results.Count);
            // Both halves parsed cleanly (not split into garbage) — guards split quality.
