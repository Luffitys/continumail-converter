...
public class PstWriterAttachmentTests
{
    ...
    {
        string tempDir = Path.Combine(Path.GetTempPath(), "mbox2pst-attach-" + Guid.NewGuid());
        ...
        try
        {
            ...
            var planned = new List<PlannedMessage> { new() { TargetFolderName = "Inbox", Message = message } };
