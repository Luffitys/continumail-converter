...
namespace PSTFileFormat
{
    public class PSTFolder : PSTNode
    {
        ...
        private Nullable<int> m_unreadCount; // Parallel to m_contentCount; maintained by AddMessage
        ...
        public override void SaveChanges()
        {
            if (m_contentCount.HasValue)
            {
                ...
                parentHierarchyTable.SetInt32Property(rowIndexOfFolder, PropertyID.PidTagContentUnreadCount, m_unreadCount ?? 0);
        ...
        public void AddMessage(MessageObject message)
        {
            ...
            bool isUnread = (msgFlags & 0x0001) == 0;
            if (isUnread)
            {
                int unreadCount = (PC.GetInt32Property(PropertyID.PidTagContentUnreadCount) ?? 0) + 1;
                PC.SetInt32Property(PropertyID.PidTagContentUnreadCount, unreadCount);
