...
namespace PSTFileFormat
{
    public class AllocationMapPage : Page // AMAPPAGE
    {
        ...
        /// <summary>
        /// Like GetMaxContiguousSpace, but only counts a free run if it starts at bit 0
        /// of a rgbAMapBits byte — i.e. at a 512-byte-aligned file offset. This matches
        /// the constraint FindContiguousSpace applies when pageAligned == true.
        /// </summary>
        /// <returns>Max contiguous, byte-aligned-start free space in bytes</returns>
        public int GetMaxAlignedContiguousSpace()
        {
            int freeLength = 0;
            int max = 0;
            ...
            for (int byteIndex = 0; byteIndex < rgbAMapBits.Length; byteIndex++)
            {
                for (int bitNumber = 0; bitNumber < 8; bitNumber++)
                {
                    ...
                    // The MSB represents the first bit in the map
                    int bitOffset = 7 - bitNumber;
                    if (bitNumber > 0 && freeLength == 0)
                    {
                        // A free run may only START at bit 0 of a byte (512-byte aligned).
                        break; // not in the middle of a free run, scan next byte
                    }
