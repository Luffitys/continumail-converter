...
{
    public class PSTCRCCalculation
    {
        ...
        public static uint ComputeCRC(byte[] pv, int startOffset, int cbLength)
        {
            byte[] temp = new byte[cbLength];
            Array.Copy(pv, startOffset, temp, 0, cbLength);
            return ComputeCRC(temp, cbLength);
        }

        public static uint ComputeCRC(byte[] pv, int cbLength)
        { 
            // Seed CRC is 0 for PST related calculations
            return ComputeCRC(0, pv, cbLength);
        }

        private static uint ComputeCRC(uint dwCRC, byte[] pv, int cbLength)
        {
            int i;
            uint dw2nd32;
            int pbBuffer = 0; // offset to the current position in pv

            int cbRunningLength = (cbLength / 8) * 8;
            int cbEndUnalignedBytes = cbLength - cbRunningLength;

           for(i=0; i < cbRunningLength/8; ++i)
           {
              dwCRC ^= LittleEndianConverter.ToUInt32(pv, pbBuffer);
              ...
              dw2nd32 = LittleEndianConverter.ToUInt32(pv, pbBuffer);
              dwCRC =   dwCRC ^ 
           ...
           {
               dwCRC = CrcTableOffset32[(dwCRC ^ pv[pbBuffer]) & 0x000000FF] ^ (dwCRC >> 8);
               pbBuffer++;
           }
